...
export const PatternCard: React.FC<PatternCardProps> = ({ pattern, onClick }) => {
  const getColorClass = (type: string) => {
    switch (type) {
      case 'Reversal': return 'bg-indigo-50 text-indigo-600';
      case 'Continuation': return 'bg-emerald-50 text-emerald-600';
      case 'Candlestick': return 'bg-amber-50 text-amber-600';
  ...
  const getBadgeClass = (type: string) => {
    switch (type) {
      case 'Reversal': return 'bg-indigo-100 text-indigo-700';
      case 'Continuation': return 'bg-emerald-100 text-emerald-700';
      case 'Candlestick': return 'bg-amber-100 text-amber-700';
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-3">
        <div className={`p-2 rounded-xl ${getColorClass(pattern.type)}`}>
      ...
      <h3 className="text-lg font-display font-bold text-slate-800 mb-2 group-hover:text-indigo-600 transition-colors">
        {pattern.name}
      </h3>
